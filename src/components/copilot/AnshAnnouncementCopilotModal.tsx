...
const SUGGESTIONS = [
  {
    ...
    text: "Draft an announcement informing the team about the upcoming Diwali office holidays from November 1st to November 3rd.",
  },
  {
    label: "Milestone Celebration",
    text: "Write a message celebrating our team reaching 10,000 active retail users and thanking everyone for their hard work.",
  },
  {
    label: "Policy Update",
    text: "Create an announcement regarding the new flexible work timings policy starting from next Monday.",
  },
  {
    label: "System Maintenance",
    text: "Notify the team about scheduled server maintenance this Saturday from 10 PM to 12 AM IST, expecting minor downtime.",
  },
];

type AnshAnnouncementCopilotModalProps = {
  open: boolean;
  onClose: () => void;
  onGenerated: (title: string, body: string) => void;
};

export function AnshAnnouncementCopilotModal({
  open,
  onClose,
  onGenerated,
}: AnshAnnouncementCopilotModalProps) {
  const titleId = useId();
      ...
      try {
        ...
        if (data.success && data.title && data.body) {
          ...
          if (active) {
            ...
            onGenerated(data.title, data.body);
          }
          return;
        }
        throw new Error("Invalid response format");
      } catch (err) {
        console.error("Announcement Copilot failed:", err);
        await new Promise((resolve) => setTimeout(resolve, 1600));
        if (active) {
          // Simple fallback drafting
          const textExcerpt = prompt.substring(0, 35) + "...";
          onGenerated("Draft Announcement", `${prompt}\n\nDraft generated by Copilot.`);
        }
      } finally {
        clearTimeout(t1);
        clearTimeout(t2);
          ...
          <div className="flex shrink-0 items-center justify-between border-b border-zinc-150 px-6 py-4 dark:border-white/[0.06]">
            <div className="flex items-center gap-2.5">
              ...
              <h2 id={titleId} className="font-heading text-lg font-bold text-zinc-900 dark:text-zinc-50">
                Draft Announcement with Copilot
              </h2>
            </div>
            <button
              onClick={onClose}
              ...
              <div className="space-y-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">
                    Prompt describing announcement details
                  </label>
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="E.g., Write a policy update announcement regarding new flexible work hours starting next month."
