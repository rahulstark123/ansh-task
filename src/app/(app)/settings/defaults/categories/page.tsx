...
export default function CategoriesSettingsPage() {
  ...
  const handleDeleteCategory = async (catToDelete: string) => {
    if (customCategories.length <= 1) {
      setErrorMessage("Workspace must have at least one category.");
      setTimeout(() => setErrorMessage(""), 3000);
      return;
    }
    ...
    if (success) {
      setToastMessage(`Category "${catToDelete}" deleted.`);
      setShowToast(true);
      setTimeout(() => setShowToast(false), 4000);
    }
  };
    ...
    <div className="relative">
      <div className="flex flex-col gap-1.5 pb-6 border-b border-zinc-200/60 dark:border-white/5">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400">
          Create, view, and delete categories for organizing tasks within the workspace.
        </p>
      </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-white/[0.06] dark:bg-zinc-900/50">
            <h3 className="text-xs font-bold text-zinc-800 dark:text-zinc-200 uppercase tracking-wider mb-4">
              Workspace Categories ({customCategories.length})
            </h3>
                ...
                {customCategories.map((cat) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-50 dark:bg-zinc-800 text-zinc-400 group-hover:text-indigo-500 transition-colors">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="opacity-0 group-hover:opacity-100 rounded-lg p-1.5 text-zinc-400 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950/20 dark:hover:text-rose-400 transition-all"
                      title={`Delete ${cat}`}
                    >
